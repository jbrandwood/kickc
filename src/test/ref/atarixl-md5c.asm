...
.file [name="atarixl-md5c.xex", type="bin", segments="XexFile"]
.segmentdef XexFile
.segment XexFile
// Binary File Header
.byte $ff, $ff
...
// memcpy(void* zp($d9) destination, byte* zp($b0) source, word zp($80) num)
memcpy: {
    .label src_end = $d6
    .label dst = $d9
    .label src = $b0
    .label destination = $d9
    .label source = $b0
    .label num = $80
...
// printf_uchar(byte register(X) uvalue, byte zp($af) format_zero_padding)
printf_uchar: {
...
// memset(void* zp($b2) str, word zp($d9) num)
memset: {
    .label end = $d9
    .label dst = $b2
    .label str = $b2
...
// uctoa(byte register(X) value, byte* zp($b2) buffer)
uctoa: {
    .label digit_value = $d8
    .label buffer = $b2
    .label digit = $b4
    .label started = $b5
...
// printf_number_buffer(byte zp($d5) buffer_sign, byte zp($af) format_zero_padding)
printf_number_buffer: {
    .const format_min_length = 2
    .label buffer_digits = printf_buffer+OFFSET_STRUCT_PRINTF_BUFFER_NUMBER_DIGITS
    .label __19 = $80
    .label buffer_sign = $d5
    .label format_zero_padding = $af
    .label padding = $b4
...
// uctoa_append(byte* zp($b2) buffer, byte register(X) value, byte zp($d8) sub)
uctoa_append: {
    .label buffer = $b2
    .label sub = $d8
    ldy #0
...
// printf_padding(byte zp($d8) pad, byte zp($b5) length)
printf_padding: {
    .label i = $b6
    .label length = $b5
    .label pad = $d8
