...
// bitmap_line(byte zp($11) x0, byte zp($1c) x1, byte register(X) y0, byte zp($17) y1)
bitmap_line: {
    .label xd = $10
    .label x0 = $11
    .label x1 = $1c
...
// bitmap_line_xdyi(byte zp($16) x, byte zp($17) y, byte zp($11) x1, byte zp($10) xd, byte zp($c) yd)
bitmap_line_xdyi: {
    .label x = $16
    .label y = $17
    .label x1 = $11
...
// bitmap_line_ydxi(byte zp($1d) y, byte zp($1c) x, byte zp($1b) y1, byte zp($1a) yd, byte zp($19) xd)
bitmap_line_ydxi: {
    .label y = $1d
    .label x = $1c
    .label y1 = $1b
...
// bitmap_line_xdyd(byte zp(3) x, byte zp(4) y, byte zp($25) x1, byte zp($24) xd, byte zp($23) yd)
bitmap_line_xdyd: {
    .label x = 3
    .label y = 4
    .label x1 = $25
...
// bitmap_line_ydxd(byte zp($a) y, byte zp(9) x, byte zp(8) y1, byte zp(7) yd, byte zp(6) xd)
bitmap_line_ydxd: {
    .label y = $a
    .label x = 9
    .label y1 = 8
...
// print_str_lines(byte* zp($1e) str)
print_str_lines: {
    .label str = $1e
    lda #<menu.SCREEN
